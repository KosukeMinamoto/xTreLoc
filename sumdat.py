...
def get_args():
	...
	parser.add_argument("--dat_dir", "-d",
						type=str,
						required=True,
						help="Directory of dat files")
	parser.add_argument("--map_type", "-m",
						type=str,
						required=False,
...
def gnuplot_map(lat_min: float, lat_max: float, lon_min: float, lon_max: float):
	f = open("view.plt", "w")
	f.write("set term pdfcairo font 'Times,10' size 6,6\n")
	...
	f.write("set xlabel 'Longitude' font ',10' off 0,0.5 tc rgb 'dark-gray'\n")
	f.write("set ylabel 'Latitude' font ',10'  tc rgb 'dark-gray'\n")
	...
	f.write("     'catalog.list' using 3:2:($6/111):($5/111):4 with xyerrorbars pt 7 lc palette,\\\n")
	f.write("     'station.tbl' using 3:2 with points pt 2 lc rgb 'black'\n")
...
def python_map(lat_min: float, lat_max: float, lon_min: float, lon_max: float):
	...
	m.drawmeridians(np.arange(lon_min, lon_max, 0.5))
	m.drawparallels(np.arange(lat_min, lat_max, 0.5))
	m.scatter(df["lon"], df["lat"], c=df["dep"], cmap="jet", s=10)
	fig.tight_layout()
	plt.show()
...
def gmt6_map(lat_min: float, lat_max: float, lon_min: float, lon_max: float):
	f = open("view.gmt", "w")
	f.write("#!/bin/bash\n")
	f.write("range=" + str(lat_min) + "/" + str(lat_max) + "/" + str(lon_min) + "/" + str(lon_max) + "\n")
	f.write("file=$2\n")
	f.write("gmt begin\n")
	f.write("gmt basemap -R$range -Jm12 -Bafg -B+t'$file'\n")
	f.write("awk '{print $3,$2,$4}' $file | gmt plot -Sc0.1c -Gred -W0.5p,black\n")
	f.write("gmt end show\n")
	f.close()
	os.system("bash view.gmt " + str(lat_min) + "/" + str(lat_max) + "/" + str(lon_min) + "/" + str(lon_max) + " catalog.list")
