...
**実装に即したクイック参照**  
- **設定**: `config.json` のトップレベルで **`io`**（モード別入出力）と **`params`**（モード別パラメータ）を使用。`demo/locating_example/config.json` を参照。  
- **CLI**: `java -jar …/xTreLoc-CLI-1.0-SNAPSHOT.jar <MODE> [config.json]` — 設定ファイルは省略可、省略時は `config.json`。  
- **デモ**: サンプルデータと設定は `demo/locating_example/` にあり。CLI はそのディレクトリで `config.json` を指定して実行する。
...
**ビルドオプション:**
- GUI版のみ: `mvn clean package -Pgui`
- CLI版のみ: `mvn clean package -Pcli`
- TUI用のプロファイルはなく、デフォルトの `mvn clean package` で GUI/CLI/TUI の各JARが `target/` に生成される。
...
Maven の場合は、対象 OS で `mvn clean package -Plinux` / `-Pwindows` / `-Pmacos` で JAR をビルドし、ネイティブ形式は上記 Gradle タスクか `jpackage` の手動実行で作成してください。
...
- **2行目**: `緯度誤差 経度誤差 深さ誤差 RMS走時差残差`
  - 誤差推定値の意味はモードにより異なる（カタログCSVの xerr/yerr/zerr と同様。GRDは0）。
...
- `<MODE>`: GRD, LMO, MCMC, DE, TRD, CLS, または SYN のいずれか
- `[config.json]`: 設定ファイルのパス（省略時はカレントディレクトリの `config.json`）
...
- **DEモード**: 差分進化法による震源決定. グローバル最適化であり初期値依存を弱める. LMOと同様に個々のイベントごとに最適化を行う.
...
```bash
java -jar build/libs/xTreLoc-CLI-1.0-SNAPSHOT.jar DE config.json
```

指定されたディレクトリ内のすべての`.dat`ファイルに対して, 差分進化法による震源決定を実行する. グローバル最適化のため初期値依存が弱く, LMOの代替として利用できる.
...
```bash
java -jar build/libs/xTreLoc-CLI-1.0-SNAPSHOT.jar
```

無効なモード名（例: `1`）を指定した場合もヘルプとエラーメッセージを表示して終了する（config.json は読み込まない）.
...
- **カレントディレクトリ**: CLI の場合は **`demo/locating_example/`** で `config.json` を指定して実行する（設定内のパスは CWD 相対）。
- `demo/locating_example/` 内のサンプルデータ:
  - `catalog_ground_truth.csv`: 正解値カタログ
  - `station.tbl`: 観測点ファイル
- Gnuplot（可視化用、任意）

- **設定ファイル（実装）**: アプリケーションはトップレベルで **`io`**（モード別入出力: `datDirectory`, `outDirectory`, `catalogFile`）と **`params`**（モード別ソルバーパラメータ）を使用する。動作例は **`demo/locating_example/config.json`** を参照。従来の `modes` / `solver` 構造は使用しない。CLS の `doClustering`, `calcTripleDiff`, `useBinaryFormat` は `params.CLS` に記述する。
...
1. モードを選択: **CLS**
2. 設定: 
   - **カタログファイル**: `./demo/dat/catalog.csv` (または`dat-lmo, `dat-mcmc`)
