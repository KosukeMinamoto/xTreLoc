...
本ソフトウェアは、**相互相関により得られる走時差を最適化することで震源を決定する**震源決定ソフトウェアである。
...
**クイック参照:** [Configuration (config.json)](#configuration-configjson) · [CLIモード](#cliモード) · [デモデータセットチュートリアル](#デモデータセットチュートリアル)（`demo/locating_example/`）。
...
# fat JAR（ランチャー・GUI・CLI・TUI）を一括ビルド
mvn clean package
```

ビルドされたJARファイルは `target/` に配置される（Maven Shade プラグイン、`package` 1回で次の4つが生成される）: 
- `xTreLoc-1.0-SNAPSHOT.jar`（起動時に GUI / TUI / CLI を選ぶランチャー）
- `xTreLoc-GUI-1.0-SNAPSHOT.jar`（GUI）
- `xTreLoc-CLI-1.0-SNAPSHOT.jar`（CLI）
- `xTreLoc-TUI-1.0-SNAPSHOT.jar`（TUI）

**注:** `pom.xml` に `-Pgui` / `-Pcli` のようなプロファイルはない。GUI または CLI だけの JAR を個別に作る場合は Gradle の `guiJar` / `cliJar` / `tuiJar` を使う。

#### プラットフォーム別ビルド (Linux / Windows / macOS)

JAR のビルド方法は全プラットフォームで同じです。**ネイティブな app-image やインストーラ**を作成する場合は、**対象の OS 上で**次の Gradle タスクを実行してください（JDK 20 以上と `./gradlew build` 済みが前提）。

| OS | Gradle タスク | 説明 |
|----|----------------|------|
| **Linux** | `./gradlew createLinuxApp` | app-image を `build/dist/` に生成。続けて `createLinuxTarball` で `.tar.gz` 化。 |
| **Windows** | `./gradlew createWindowsApp` | app-image を生成。`.exe` インストーラは `createWindowsExe`（要 WiX 3+）。 |
| **macOS** | `./gradlew createApp` | .app バンドルを生成。`.dmg` は `createDmg`。 |

Windows では `./gradlew` の代わりに `gradlew.bat` を使用（例: `gradlew.bat createWindowsApp`）。

Maven の `mvn clean package` は OS に依存せず同じ JAR 一式を生成する。`-Plinux` / `-Pwindows` / `-Pmacos` は `pom.xml` 上のプレースホルダであり、JAR の内容や種類は変えない。ネイティブ形式は上記 Gradle タスク、または `pom.xml` の `-PcreateApp` / `-PcreateDmg` など、`jpackage` の手動実行で作成する。
...
スペース区切り6列（コード上の順序は `StationRepository` と同じ）: 
```
code latitude longitude H Pc Sc
```
（`H` は観測点の標高を**メートル**で表す。下向きを正とし、海抜上は負。内部では `Station` の深度として保持され、震源の深さ km 換算に用いられる。）
...
- **Pc**: 理論走時差に足す P 波の観測点補正（秒）**第5列**（`code 緯度 経度 H` の次）
- **Sc**: 理論走時差に足す S 波の観測点補正（秒）**第6列**
...
- **REF**: TRDモードにおいて固定される参照イベント (詳細は [Reference Events (REF)](#reference-events-ref))
...
- **2行目**: `緯度誤差 経度誤差 深さ誤差 RMS走時差残差`
  - 誤差推定値の意味はカタログ CSV の xerr/yerr/zerr と同様（LMO / MCMC / DE / TRD / GRD などモード依存。GRD は 0）。
- **3行目以降**: `観測点1 観測点2 走時差 重み`
...
**フィールド説明**: 
- **走時差**: S波についての走時差. 観測点st1とst2における到達時刻 $T_st1$ と$T_st2$ について, $T_st2 - T_st1$ の値に対応する. 
...
SYNモードでは重みはすべて `1.0`。**GRD / LMO / MCMC / DE** の実行後は、走時差残差から `|1 / (残差 + ε)|` で重みが更新される（ソースの `HypoUtils.residual2weight`）。例: 残差 −2 s なら重み `0.5`。`config.json` の `threshold` は読み込み時に重みと比較して検測を採用するか判定する（`PointsHandler`）。後段の LMO / MCMC / DE / CLS / TRD でも `threshold` で採用ペアを制御できる。**「閾値超えが4本未満なら専用のエラーを出す」といったチェックは Java 実装にはない**；採用データが極端に少ないと数値・最適化の失敗につながる場合がある。

---

## Configuration (config.json)

GUI / CLI / TUI が読む JSON は、Java の `AppConfig` に沿ったトップレベル構造である。

- **`io`**: モード名（`GRD`, `LMO`, …）ごとの `datDirectory`, `outDirectory`, `catalogFile` など。
- **`params`**: モード別のソルバーパラメータ（例: CLS の `doClustering`, `calcTripleDiff`, `useBinaryFormat` は `params.CLS` 配下）。

パスはすべてプロセスの**カレントディレクトリ**相対で解釈される。動作例は **`demo/locating_example/config.json`** を参照（旧来の `modes` / `solver` のみの構成は用いない）。
...
- **Solver**: モード・観測点ファイル・入出力ディレクトリ・閾値を設定しバッチ実行する。論理構造は CLI と同じ `config.json`（`io` / `params`）である。
- **Settings**: フォント・配色・ログレベル・自動更新などアプリ設定（`~/.xtreloc/settings.json`）。
...
- **カタログエクスポート**: カタログ行を CSV で保存（`CsvExporter`）。
- **レポートエクスポート**: 数値列ごとの要約統計（最小・最大・平均・中央値・標準偏差・四分位・IQR など）を `.txt` に出力（`ReportPanel.writeReport`）。
...
- **波形表示**: 複数観測点の波形を同時に表示
- **フィルタリング**: バンドパスフィルタを適用して波形を処理。波形読み込み時の既定は **1–16 Hz**（`WaveformPickingPanel`）。高域のテキスト欄の初期表示が異なる場合があるので、表示と適用周波数を一致させるには **Update Filter** を押す。
- **ピッキング操作**: 
  - 左クリック: P波到着時刻のピッキング
  - 右クリック: S波到着時刻のピッキング
...
1 回の起動で 1 つの `<MODE>` を処理するバッチ用エントリポイント。第 2 引数の JSON は省略時 `config.json`（カレントディレクトリ）。入出力パスとソルバー設定は [Configuration (config.json)](#configuration-configjson) を参照。モードの役割とワークフローは [はじめに](#はじめに) と [デモデータセットチュートリアル](#デモデータセットチュートリアル) にまとめる。
...
Maven でビルドした場合は `target/xTreLoc-CLI-1.0-SNAPSHOT.jar` を使う。
...
例: `./gradlew runCLI -PcliArgs="GRD demo/locating_example/config.json"`

- `<MODE>`: `GRD`, `LMO`, `MCMC`, `DE`, `CLS`, `TRD`, `SYN` のいずれか。
...
`-h` / `--help` で用法を表示して終了する。引数なしの挙動は端末が対話的かどうかにより異なる（`XTreLocCLI` 参照）。無効なモード名を渡した場合も用法が表示され、`config.json` は読み込まれない。
...
このチュートリアルでは、**`demo/locating_example/`** 内のサンプルデータで xTreLoc を実行する手順を説明する。

**重要**: CLI では **`demo/locating_example/`** をカレントにして、その中の `config.json` を指定して実行する。設定ファイル内のパスはカレントディレクトリ相対である。プロジェクトルートから `demo/locating_example/config.json` を指定して実行する場合は、パスがプロジェクトルート相対になる点に注意。

### 準備

- Java 20 以上とビルド済み JAR（[インストールとビルド](#インストールとビルド)）
- **カレントディレクトリ**: 以下の CLI 例では **`demo/locating_example/`** をカレントにし、その中の `config.json` を渡す（設定内パスは CWD 相対。ルートから別パスで起動する場合は冒頭の注意を参照）
- 同ディレクトリの `catalog_ground_truth.csv`, `station.tbl`
- 任意: [ステップ4](#ステップ4-結果の可視化) 用の Gnuplot / MATLAB / Python / GMT

設定の全体は [Configuration (config.json)](#configuration-configjson)、実例は **`demo/locating_example/config.json`**。

### デモデータセット構造

デモは **`demo/locating_example/`** に配置されている。必要なら出力用ディレクトリを作成する:
...
├── catalog_ground_truth.csv  # SYNモード用正解値カタログ
├── station.tbl               # 観測点ファイル
├── config.json               # io & params 形式の設定（上記参照）
├── utils/
│   ├── map_njt.plt           # Gnuplot用
│   ├── map_njt.m             # MATLAB用
│   ├── map_njt.py            # Python用
│   └── map_njt.sh            # GMT用
└── dat/                      # 入力 .dat ファイル（SYNモードで生成）
```
...
ステップ1: SYN → .dat 生成
  ↓
ステップ2: GRD+LMO または MCMC → catalog.csv
  ↓
ステップ3: CLS → クラスタ + triple_diff_*.bin → TRD → 再配置カタログ
  ↓
ステップ4: 可視化（Gnuplot / MATLAB / Python / GMT または GUI）
```

モードの概要・注意（TRD の入力品質、ブートストラップ未実装など）は [はじめに](#はじめに) を参照。
...
1. 設定: 
   ...
   - **出力ディレクトリ**: `./demo/dat-lmo` (事前に存在している必要がある)
   - **パラメータ**: 
     - Parallelization (numJobs): 並列化計算におけるジョブ数 (>1)
     - Weight Threshold (threshold): weight値のスクリーニング (0であれば全ての走時データを用いる)
     - Maximum Depth: 探索される深さの限界. 一方で, 浅さの限界は最も深い観測点の深さ. 
...
1. モードを選択: **CLS**
2. 設定: 
   - **カタログファイル**: `./demo/dat/catalog.csv`（または `dat-lmo` / `dat-mcmc` の `catalog.csv`）
...
2. 設定: 
   ...
   - **パラメータ**: 
     - Parallelization (numJobs): 並列化計算におけるジョブ数 (>1)	
     - Weight Threshold (threshold): weight値のスクリーニング (0であれば全ての走時データを用いる)
     - Maximum Depth: 探索される深さの限界. 一方で, 浅さの限界は最も深い観測点の深さ. <u>GRD, LMO, MCMC, DE モードとは異なり, これ以深 (または観測点よりも浅い) に更新された場合には決定不良と判断され, Errタグが付与される.</u>
     ...
     - **参照イベント (REF)**: 任意。定義と作り方は [Reference Events (REF)](#reference-events-ref) を参照。
     - **LSQR**（`atol`, `btol`, `conlim`, `iter_lim`, `showLSQR`）: 内部三重差ソルバーの制御。既定値（概ね `1e-6` / `1e-6` / `1e8` / `1000` / `true`）で足りない場合のみ調整する。
...
TRD で `mode` が `REF` の行は**再決定で動かさない**基準点であり、三重差分には参加する。カタログの手編集、または SYN 用正解カタログで `REF` を指定して生成する。CLS では他イベントと同様にクラスタへ入る。
...
- GRD&LMO結果: ![GRD&LMO Results](gnu_syn_grd_lmo.png)
