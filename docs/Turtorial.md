...
**Note on Error Estimates**: The meaning of error estimates (xerr, yerr, zerr) differs depending on the location mode:
- **LMO Mode**: Standard errors calculated from the covariance matrix using the Jacobian matrix. Represents the formal uncertainty based on the linearized least-squares solution.
- **MCMC Mode**: Standard deviation of the posterior distribution samples (after burn-in). Represents the Bayesian uncertainty estimate from the sampled distribution.
- **DE Mode**: Standard deviation of the final generation population distribution. Represents the spread of candidate solutions around the best individual, indicating the convergence and uncertainty of the optimization.
- **TRD Mode**: Standard errors from the LSQR solver's variance matrix. Represents the formal uncertainty in the linearized triple-difference relocation problem.

These different error estimates reflect the different statistical frameworks and optimization methods used in each mode, and should be interpreted accordingly.
...
- **Line 2**: `latitude_error longitude_error depth_error RMS_travel_time_difference_residual`
  - The error estimates (latitude_error, longitude_error, depth_error) have different meanings depending on the mode (see [Error Estimates](#error-estimates) section below)
...
The error estimates in Line 2 (`latitude_error`, `longitude_error`, `depth_error`) have different meanings depending on the location mode:

- **LMO Mode**: Standard errors calculated from the covariance matrix $\mathbf{C} = \sigma^2 (\mathbf{J}^T \mathbf{J})^{-1}$, where $\mathbf{J}$ is the Jacobian matrix and $\sigma$ is the RMS residual. These represent formal uncertainties based on the linearized least-squares solution.

- **MCMC Mode**: Standard deviation of the posterior distribution samples (after burn-in). These represent Bayesian uncertainty estimates from the sampled distribution, reflecting the spread of likely hypocenter locations.

- **DE Mode**: Standard deviation of the final generation population distribution. These represent the spread of candidate solutions around the best individual, indicating both the convergence quality and the uncertainty of the optimization result.

- **TRD Mode**: Standard errors from the LSQR solver's variance matrix. These represent formal uncertainties in the linearized triple-difference relocation problem, based on the design matrix and residual statistics.

**Important**: These different error estimates reflect different statistical frameworks and should not be directly compared across modes. For example, MCMC errors typically represent a broader uncertainty distribution, while LMO errors represent formal linearized uncertainties that may underestimate true errors in non-linear problems.
...
- `<MODE>`: One of GRD, LMO, MCMC, TRD, CLS, or SYN (DE is not implemented in CLI; use GUI or TUI for DE.)
- `[config.json]`: Path to configuration file (Default: `config.json`)

### Available Modes

**Note**: DE (Differential Evolution) mode is available in GUI and TUI only; it is not implemented in CLI.
...
├── station.tbl                # Station file
├── map_njt.plt                # Gnuplot visualization script
├── map_njt.m                  # MATLAB visualization script
├── map_njt.py                 # Python visualization script
├── map_njt.sh                 # GMT visualization script
...
- GRD&LMO results: ![GRD&LMO Results](gnu_syn_grd_std.png)
