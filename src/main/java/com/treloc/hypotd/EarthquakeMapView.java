...
public class EarthquakeMapView {
	...
	public EarthquakeMapView(ConfigLoader appConfig) throws Exception {
		...
		List<EarthquakeData> earthquakes = loadEarthquakeData(catalogFile);
		if (!earthquakes.isEmpty()) {
			addHypocenterLayer(earthquakes);
			addStationLayer();
			addGraticules(earthquakes);
			setBound(earthquakes);
		}
	 ...
	 * @return List of EarthquakeData objects
	 */
	private List<EarthquakeData> loadEarthquakeData(String catalogFile) {
		...
		try (BufferedReader br = new BufferedReader(new FileReader(catalogFile))) {
			...
			while ((line = br.readLine()) != null) {
				...
				earthquakes.add(new EarthquakeData(
						new Coordinate(longitude, latitude),
						parts[0],
						depth));
			}
		} catch (Exception e) {
			LOGGER.warning("Error loading earthquake data: " + e.getMessage());
		}
		return earthquakes;
	}
	 ...
	 *
	 * @param earthquakes List of earthquake data to display
	 */
	private void addHypocenterLayer(List<EarthquakeData> earthquakes) {
		try {
			...
			for (int i = 0; i < earthquakes.size(); i++) {
				SimpleFeature feature = DataUtilities.template(type);
				feature.setDefaultGeometry(
						new GeometryFactory().createPoint(earthquakes.get(i).getCoordinate()));
				feature.setAttribute("time", earthquakes.get(i).getTimeString());
				feature.setAttribute("depth", earthquakes.get(i).getDepth());
				featureCollection.add(feature);
			}
	 ...
	 *
	 * @param earthquakes List of earthquakes to determine bounds from
	 */
	private void setBound(List<EarthquakeData> earthquakes) {
		double minLat = earthquakes.stream().mapToDouble(e -> e.getCoordinate().y).min().orElse(0);
		double maxLat = earthquakes.stream().mapToDouble(e -> e.getCoordinate().y).max().orElse(0);
		double minLon = earthquakes.stream().mapToDouble(e -> e.getCoordinate().x).min().orElse(0);
		double maxLon = earthquakes.stream().mapToDouble(e -> e.getCoordinate().x).max().orElse(0);
	 ...
	 *
	 * @param earthquakes List of earthquakes to determine grid extent
	 */
	private void addGraticules(List<EarthquakeData> earthquakes) {
		try {
			double minLat = earthquakes.stream().mapToDouble(e -> e.getCoordinate().y).min().orElse(0);
			double maxLat = earthquakes.stream().mapToDouble(e -> e.getCoordinate().y).max().orElse(0);
			double minLon = earthquakes.stream().mapToDouble(e -> e.getCoordinate().x).min().orElse(0);
