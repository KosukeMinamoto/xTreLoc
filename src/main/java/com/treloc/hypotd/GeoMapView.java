...
/**
 * GeoMapView
 * This class is used to visualize the hypocenter location.
 * 
 * @author K.M.
 * @since 2025-02-22
 * @version 0.1
...
public class GeoMapView extends ApplicationFrame {
	...
	public GeoMapView(ConfigLoader appConfig) {
		super("MapView");
		...
		// Seismicity info
		Cluster<Point> catalogCluster = loadPointsFromCatalog(catalogFile);
		XYSeries hypSeries = loadEventData(catalogCluster);
	...
	private XYSeries loadEventData(Cluster<Point> catalogCluster) {
		XYSeries hypSeries = new XYSeries("Hypocenter");
		for (Point point : catalogCluster.getPoints()) {
	 ...
	 */
	public Cluster<Point> loadPointsFromCatalog(String catalogFile) {
