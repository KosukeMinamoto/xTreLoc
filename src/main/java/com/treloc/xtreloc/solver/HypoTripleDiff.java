...
public class HypoTripleDiff extends SolverBase {
    ...
    public void start(String datFile, String outputFile) throws TauModelException {
        ...
        try {
            ...
            SolverLogger.info("TRD: Loading points from catalog: " + catalogPathToUse);
            ...
            SolverLogger.info("TRD: Loaded " + allPoints.size() + " points from catalog");
            ...
            SolverLogger.info("TRD: Clustering detected. Found " + clusterIds.size() + " unique cluster ID(s)");
            ...
            logger.info("Clustering confirmed. Found " + clusterIds.size() + " cluster(s) (including noise). " +
                       "Processing clusters: " + clusterIds.stream()
                           ...
                           .map(String::valueOf)
                           .collect(java.util.stream.Collectors.joining(", ")));
            SolverLogger.info("TRD: Clustering confirmed. Found " + clusterIds.size() + " cluster(s). Processing: " +
                clusterIds.stream().filter(cid -> cid > 0).map(String::valueOf).collect(java.util.stream.Collectors.joining(", ")));
