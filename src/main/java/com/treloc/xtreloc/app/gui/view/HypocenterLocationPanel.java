 ...
 * mode-specific parameters, execution log, and convergence/residual plots.
 *
 * @see HypoGridSearch
...
public class HypocenterLocationPanel extends JPanel {
    ...
    private void initComponents() {
        ...
        rightPanel.setMinimumSize(new Dimension(360, 280));
        ...
        // Use proportional location so right panel (Execution/Convergence log) gets space on Linux/Ubuntu
        splitPane.setDividerLocation(0.5);
        SwingUtilities.invokeLater(() -> {
            if (splitPane.getWidth() > 0) splitPane.setDividerLocation(0.5);
        });
    ...
    /**
     * Updates the Convergence Log panel for the given mode.
     * SYN shows a placeholder message; CLS shows k-distance plot; others show residual plot.
     *
    ...
    private JPanel createParameterPanel() {
        ...
        // Create title panel with help button only (section border already shows "Parameters")
        ...
        // No custom border title - using createSectionBorder("Parameters")
    ...
    private JPanel createConvergenceLogPanel() {
        JPanel panel = new JPanel(new BorderLayout());
        AppPanelStyle.setPanelBackground(panel);
        panel.setBorder(AppPanelStyle.createSectionBorder("Convergence Log (LMO/MCMC/DE/TRD)"));
    ...
    /**
     * Registers the convergence callback so that residual and likelihood updates
     * from solvers are reflected in the residual plot and convergence log in real time.
     */
    ...
    private void restoreModeStateFromCache(String mode, String sourceModeForCopy) {
        ...
        if (cache == null || cache.isEmpty()) {
            // Copy from source mode (e.g. the mode we're leaving) so target gets its params
    ...
    private java.util.List<String> validateBeforeExecute(String selectedMode) {
        ...
        if ("GRD".equals(selectedMode)) {
            ...
            // LMO has many optional params; no mandatory empty check needed for basic run
        } else if ("MCMC".equals(selectedMode)) {
    ...
    private static Color colorForLogMessage(String message) {
        ...
        if (m.startsWith("ERROR:") || m.startsWith("Error:")) {
            return new Color(255, 100, 100);   // red for errors
        }
    ...
    private void loadLogHistory() {
        ...
        try {
            ...
            historyLines = settings.getHistoryLines();
        } catch (Exception e) {
            logger.warning("Could not load settings for log history line count, using default: " + e.getMessage());
        }
    ...
    private AppConfig buildConfigFromUI() {
        ...
        if ("GRD".equals(selectedMode)) {
            ...
            if (!rmsThresholdStr.isEmpty()) {
                try { clsSolver.put("rmsThreshold", Double.parseDouble(rmsThresholdStr)); } catch (NumberFormatException e) { /* ignore */ }
            }
            if (!locErrThresholdStr.isEmpty()) {
                try { clsSolver.put("locErrThreshold", Double.parseDouble(locErrThresholdStr)); } catch (NumberFormatException e) { /* ignore */ }
            }
    ...
    private void executeLocation() {
        ...
        currentWorker = new SwingWorker<Void, String>() {
            ...
            protected Void doInBackground() throws Exception {
                ...
                cancelButton.setEnabled(true);
                SwingUtilities.invokeLater(() -> {
                    if (rightTabbedPane != null && rightTabbedPane.getTabCount() > 0) {
                ...
                try {
                    ...
                    if (!allHypocenters.isEmpty()) {
                        File catalogFile = getOutputFileFromField();
                        if (catalogFile == null) {
            ...
            protected void done() {
                SwingUtilities.invokeLater(() -> {
                    try {
                        ...
                        if (currentWorker.isCancelled() || (e.getCause() != null && "Cancelled".equals(e.getCause().getMessage()))) {
                            appendLog("Cancelled successfully");
                        } else {
                            // Error should already be logged via publish() in doInBackground
                            StringBuilder errorMsg = new StringBuilder("Error: " + e.getMessage());
    ...
    /**
     * Displays the k-distance graph in the Convergence Log tab.
     *
    ...
    public void setConfig(AppConfig config) {
        ...
        if (config != null) {
            ...
            if (config.getModes() != null) {
                // Populate per-mode caches from config (I/O and params for each mode including SYN)
                ...
                // Apply params.CLS from config (in case only in params)
                ...
                // Apply current mode's state so the visible fields show the selected mode's I/O and params
                if (modeCombo != null) {
            ...
            // Re-apply current mode so visible I/O and param table match the selected mode
            ...
            // Refresh parameter table so it shows the values just applied (e.g. TRD params)
            updateParameterFields();
        }
    }
