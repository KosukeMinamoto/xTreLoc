...
public class CatalogTablePanel extends JPanel {
    ...
    private JCheckBox errorEllipseCheckBox;
    ...
    public CatalogTablePanel(MapView mapView) {
        ...
        JPanel catalogListToolbar = new JPanel(new FlowLayout(FlowLayout.LEFT, 5, 2));
        AppPanelStyle.setPanelBackground(catalogListToolbar);
        errorEllipseCheckBox = new JCheckBox("Error ellipse");
        errorEllipseCheckBox.setToolTipText("Show/hide error ellipses for the selected catalog on the map");
        errorEllipseCheckBox.addActionListener(e -> {
            int row = catalogListTable.getSelectedRow();
            if (row >= 0 && row < catalogInfos.size() && mapView != null) {
                com.treloc.xtreloc.app.gui.model.CatalogInfo info = catalogInfos.get(row);
                info.setShowErrorEllipse(errorEllipseCheckBox.isSelected());
                mapView.updateMultipleCatalogsDisplay();
            }
        });
        catalogListToolbar.add(errorEllipseCheckBox);
        ...
        catalogListWrapper.add(catalogListToolbar, BorderLayout.NORTH);
    ...
    private void updateErrorEllipseCheckboxForSelection() {
        if (errorEllipseCheckBox == null) return;
        int row = catalogListTable.getSelectedRow();
        if (row >= 0 && row < catalogInfos.size()) {
            com.treloc.xtreloc.app.gui.model.CatalogInfo info = catalogInfos.get(row);
            errorEllipseCheckBox.setEnabled(true);
            errorEllipseCheckBox.setSelected(info.isShowErrorEllipse());
        } else {
            errorEllipseCheckBox.setEnabled(false);
            errorEllipseCheckBox.setSelected(false);
        }
    }
    ...
    private class ColorColumnHatchRenderer extends DefaultTableCellRenderer {
        ...
        private static final Color HATCH_COLOR = new Color(255, 220, 0, 100);
        ...
        protected void paintComponent(Graphics g) {
            ...
            if (tableRef == null || lastColumn < 0) return;
            int modelCol = tableRef.convertColumnIndexToModel(lastColumn);
            if (modelCol != colorColumnIndex || colorColumnIndex < 0) return;
            int w = getWidth();
            int h = getHeight();
            Graphics2D g2 = (Graphics2D) g.create();
            g2.setColor(HATCH_COLOR);
            g2.setStroke(new BasicStroke(1f));
            for (int i = -h; i < w + h; i += HATCH_SPACING) {
