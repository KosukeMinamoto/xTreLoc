...
public class WaveformPickingPanel extends JPanel {
    ...
    private static final Logger logger = Logger.getLogger(WaveformPickingPanel.class.getName());
    ...
    public WaveformPickingPanel() {
        ...
        AppSettings settings = AppSettingsCache.snapshot();
    ...
    private JPanel createRightPanel() {
        ...
        AppPanelStyle.setPanelBackground(chartPanel);
        
        // Zoom chart panel (zoomed waveform display)
        zoomChartPanel = new JPanel(new GridLayout(1, 1));
        AppPanelStyle.setPanelBackground(zoomChartPanel);
        zoomChartPanel.setBorder(AppPanelStyle.createTitledSectionBorder("Zoom View"));
        zoomChartPanel.setPreferredSize(new java.awt.Dimension(0, 150));
        ...
        AppPanelStyle.setPanelBackground(guidePanel);
        ...
        guideLabel.setForeground(AppPanelStyle.getMutedTextColor());
    ...
    private JPanel createLeftPanel() {
        ...
        AppPanelStyle.applyThemeTo(panel);
        return panel;
    }
    ...
    private JPanel createTopPanel() {
        ...
        AppPanelStyle.setPanelBackground(panel);
        panel.setBorder(AppPanelStyle.createSectionBorder("Controls"));
        ...
        AppPanelStyle.styleTextField(inputDirField);
        ...
        try {
            ...
            logger.log(Level.FINE, "Could not set folder icon for input dir, using fallback: " + e.getMessage());
            selectInputDirButton.setText("📁");
        }
        ...
        AppPanelStyle.stylePrimaryButton(selectInputDirButton);
        ...
        try {
            ...
            logger.log(Level.FINE, "Could not set folder icon for output dir, using fallback: " + e.getMessage());
            selectOutputDirButton.setText("📁");
        }
        ...
        AppPanelStyle.stylePrimaryButton(updateButton);
        ...
        AppPanelStyle.styleSuccessButton(saveButton);
    ...
    private JPanel createDirectoryTreePanel() {
        JPanel panel = new JPanel(new BorderLayout());
        AppPanelStyle.setPanelBackground(panel);
        panel.setBorder(AppPanelStyle.createTitledSectionBorder("Directory Tree"));
        ...
        AppPanelStyle.styleTree(directoryTree);
    ...
    private JPanel createFileListPanel() {
        JPanel panel = new JPanel(new BorderLayout());
        AppPanelStyle.setPanelBackground(panel);
        panel.setBorder(AppPanelStyle.createTitledSectionBorder("SAC Files"));
        
        // File list
        fileList = new JList<>();
        fileList.setSelectionMode(ListSelectionModel.MULTIPLE_INTERVAL_SELECTION);
        AppPanelStyle.styleList(fileList);
    ...
    private void loadChildrenForNode(DefaultMutableTreeNode node, File dir) {
        ...
        if (treeRootLimit != null) {
            try {
                ...
                logger.log(Level.FINE, "Path comparison failed, skipping directory: " + e.getMessage());
                return;
            }
        }
        ...
        try {
                    ...
                    if (child.isDirectory() && !child.isHidden() && child.canRead()) {
                        ...
                        if (treeRootLimit != null) {
                            try {
                                ...
                                logger.log(Level.FINE, "Path comparison failed, skipping child: " + e.getMessage());
                                continue;
                            }
                        }
            ...
            logger.log(Level.FINE, "Permission error listing directory: " + e.getMessage());
        } catch (Exception e) {
            logger.log(Level.FINE, "Error listing directory: " + e.getMessage());
        }
    }
    ...
    private boolean hasSubdirectories(File dir) {
        ...
        try {
                    ...
                    if (child.isDirectory() && !child.isHidden()) {
                        ...
                        if (treeRootLimit != null) {
                            try {
                                ...
                                logger.log(Level.FINE, "Path comparison failed in hasSubdirectories: " + e.getMessage());
                                continue;
                            }
                        }
                        return true;
            ...
            logger.log(Level.FINE, "Permission error in hasSubdirectories: " + e.getMessage());
        } catch (Exception e) {
            logger.log(Level.FINE, "Error in hasSubdirectories: " + e.getMessage());
        }
    ...
    private void loadDirectory(File dir) {
        try {
            ...
            logger.log(Level.SEVERE, "Error loading directory: " + ex.getMessage(), ex);
        ...
        fileList.addListSelectionListener((ListSelectionEvent event) -> {
            if (!event.getValueIsAdjusting()) {
                try {
                    ...
                    logger.log(Level.WARNING, "Error loading waveforms: " + ex.getMessage(), ex);
                    JOptionPane.showMessageDialog(this,
    ...
    private void updateFilter() {
        try {
            ...
            logger.log(Level.WARNING, "Error updating filter: " + ex.getMessage(), ex);
            JOptionPane.showMessageDialog(this,
    ...
    private void updateCharts(SacTimeSeries[] waveforms) {
        ...
        ChartImageExport.installChartPopupMenu(chartPanelComponent, this,
            () -> chartPanelComponent.restoreAutoBounds(), "waveform.png");
        ...
        chartPanelComponent.addKeyListener(new KeyListener() {
            ...
            public void keyPressed(KeyEvent e) {
                ...
                AppSettings keySettings = AppSettingsCache.snapshot();
                if (code == keyCodeFromSetting(keySettings.getPickingKeyP())) pickingKeyPDown = true;
                if (code == keyCodeFromSetting(keySettings.getPickingKeyS())) pickingKeySDown = true;
            }
            @Override
            public void keyReleased(KeyEvent e) {
                int code = e.getKeyCode();
                AppSettings keySettings = AppSettingsCache.snapshot();
                if (code == keyCodeFromSetting(keySettings.getPickingKeyP())) pickingKeyPDown = false;
                if (code == keyCodeFromSetting(keySettings.getPickingKeyS())) pickingKeySDown = false;
            }
            @Override
    ...
    private void updateZoomChart(TimeSeriesCollection dataset, int seriesIndex, int itemIndex) {
        ...
        try {
            ...
            zoomChartPanelComponent.setPopupMenu(null);
            ChartImageExport.installChartPopupMenu(zoomChartPanelComponent, this,
                () -> zoomChartPanelComponent.restoreAutoBounds(), "waveform_zoom.png");
            zoomChartPanel.add(zoomChartPanelComponent);
            
        } catch (Exception ex) {
            logger.log(Level.WARNING, "Error updating zoom chart: " + ex.getMessage(), ex);
        }
    ...
    private void loadStationFile(String filePath) {
        ...
        try (java.io.BufferedReader reader = new java.io.BufferedReader(
            ...
            logger.info("Loaded " + channelMap.size() + " channel mappings from station file");
        } catch (java.io.IOException e) {
            logger.log(Level.SEVERE, "Error reading station file: " + e.getMessage(), e);
            JOptionPane.showMessageDialog(this,
    ...
    private void savePickfile() {
        ...
        try {
            ...
            logger.log(Level.WARNING, "Error reading SAC file: " + e.getMessage(), e);
            JOptionPane.showMessageDialog(this,
        ...
        try (java.io.PrintWriter writer = new java.io.PrintWriter(
            ...
            for (com.treloc.xtreloc.app.gui.service.ArrivalTime arrival : arrivalTimeManager.getArrivalTimeMap().values()) {
                ...
                if (channelNumber == null) {
                    logger.warning("No channel mapping found for " + key);
                    continue;
                }
                ...
                if (timeDiffMillis < 0) {
                    logger.warning("Arrival time is before reference time for " + station + " " + component);
                    continue;
                }
            ...
            logger.log(Level.SEVERE, "Error writing pickfile: " + e.getMessage(), e);
            JOptionPane.showMessageDialog(this,
        ...
        if (currentWaveforms != null && currentWaveforms.length > 0) {
            ...
            if (selectedIndices.length > 0 && currentFilesInDir != null) {
                try {
                    ...
                    logger.log(Level.WARNING, "Error refreshing chart annotations: " + ex.getMessage(), ex);
                }
            }
        }
    }
