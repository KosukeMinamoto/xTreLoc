...
public class ColorMapSettingsDialog extends JDialog {
	...
	private JPanel createRangePanel() {
		...
		minValueField = new JTextField(String.format(Locale.US, "%.2f", colorBarPanel.getMinValue()), 10);
		maxValueField = new JTextField(String.format(Locale.US, "%.2f", colorBarPanel.getMaxValue()), 10);
	...
	private void updateRange() {
		try {
			double newMin = parseRangeDouble(minValueField.getText());
			double newMax = parseRangeDouble(maxValueField.getText());
	...
	/** Parse min/max text; accepts dot or comma as decimal separator (independent of JVM locale). */
	private static double parseRangeDouble(String raw) throws NumberFormatException {
	...
	private void writeCPTFile(FileWriter writer) throws IOException {
		...
		for (int i = 0; i < numSteps - 1; i++) {
			...
			writer.write(String.format(Locale.US, "%.6f\t%d\t%d\t%d\t%.6f\t%d\t%d\t%d\n",
				z1, c1.getRed(), c1.getGreen(), c1.getBlue(),
				z2, c2.getRed(), c2.getGreen(), c2.getBlue()));
		}
		
		writer.write(String.format(Locale.US, "B\t%d\t%d\t%d\n",
	...
	private void writeCSVFile(FileWriter writer) throws IOException {
		...
		for (int i = 0; i < colors.length; i++) {
			...
			Color c = colors[i];
			writer.write(String.format(Locale.US, "%.6f,%d,%d,%d\n", value, c.getRed(), c.getGreen(), c.getBlue()));
		}
	}
}
