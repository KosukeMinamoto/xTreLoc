...
public class MapView {
	...
	/** Cache for symbol size from settings; -1 means not loaded. Cleared when setSymbolSize is called. */
	private int cachedSymbolSizeFromSettings = -1;
	private boolean showErrorEllipse = false;
	...
	private JToggleButton eventConnectionToggleButton;
	...
	/** Placeholder ARGB for hypocenters/stations with no finite color value (rendered fully transparent). */
	...
	/** Panel above the map: Error ellipse, Connection, loading bar (placed in Viewer tab). */
	...
	public MapView() {
		...
		try {
			javax.swing.ImageIcon logoIcon = com.treloc.xtreloc.app.gui.util.BundledImageLoader.loadImageIcon("logo.png");
		...
		// Panel above map: Error ellipse, Connection, loading bar (used in Viewer tab)
		// Icon toggles for Error ellipse and Event connection are placed next to Save figure in the status bar
		mapTopPanel = new JPanel(new FlowLayout(FlowLayout.LEFT, 10, 4));
		ViewerStatusBar statusBar = new ViewerStatusBar();
		ViewerStatusBar.setInstance(statusBar);
		mapTopPanel.add(statusBar);
	...
	private void updateMapBackground() {
		try {
			...
			logger.log(Level.WARNING, "Failed to set map background color: " + e.getMessage(), e);
		}
	}
	...
	/** Returns the panel to place above the map (Error ellipse, Connection, loading bar). */
	public JPanel getMapTopPanel() {
		return mapTopPanel;
	}
	...
	/**
	 * Reloads the map display: re-builds layers from the current data (last shown hypocenters or multiple catalogs).
	 * Use after data or style changes to refresh the map.
	 */
	...
	private void setupMouseListener() {
		...
		mapPaneComponent.addComponentListener(new java.awt.event.ComponentAdapter() {
			...
			public void componentResized(java.awt.event.ComponentEvent e) {
				javax.swing.Timer delayTimer = new javax.swing.Timer(50, ev -> {
	...
	/**
	 * Adds a shapefile to the map using an already-opened FeatureSource (e.g. from background load).
	 * Use this to avoid opening the file twice and to keep heavy I/O off the EDT.
	 *
	 * @param shp the shapefile (for layer naming)
	 * @param featureSource the opened FeatureSource
	 * @return the layer title
	 */
	public String addShapefile(File shp, org.geotools.api.data.FeatureSource featureSource) throws Exception {
		if (shp == null || featureSource == null) {
			throw new IllegalArgumentException("File and FeatureSource must be non-null");
		}
		String layerTitle = "ShapefileLayer_" + shp.getName() + "_" + System.currentTimeMillis();
		Style shapefileStyle = StyleFactory.createShapefileStyleForGeometry(featureSource);
		try {
			...
			if (layerEx.getMessage() != null && layerEx.getMessage().contains("Unable to find function")) {
				logger.log(Level.FINE, "Function evaluation error detected, trying minimal style...");
				try {
					...
					map.addLayer(layer);
					forceMapRepaint();
					logger.log(Level.FINE, "Successfully created layer with minimal style");
					return layerTitle;
				} catch (Exception minimalEx) {
					logger.log(Level.WARNING, "Even minimal style failed: " + minimalEx.getMessage(), minimalEx);
				}
			}
			logger.log(Level.SEVERE, "Failed to create FeatureLayer: " + layerEx.getMessage(), layerEx);
	...
	public String addShapefile(File shp) throws Exception {
		...
		try {
			...
			try {
				...
				logger.log(Level.FINE, "Store dispose failed: " + disposeEx.getMessage());
			}
			
			logger.log(Level.SEVERE, "Failed to add shapefile to map: " + e.getMessage(), e);
	...
	/**
	 * Sets whether to show error ellipses. When multiple catalogs are loaded, sets the flag for all catalogs (global toggle).
	 *
	...
	public void setShowErrorEllipse(boolean show) {
		...
		for (com.treloc.xtreloc.app.gui.model.CatalogInfo c : catalogInfos) {
			c.setShowErrorEllipse(show);
		}
		// When multiple catalogs are shown, always use updateMultipleCatalogsDisplay()
	 ...
	 * <p>Reverted experiment (was disabled): EDT-off layer build via {@code SwingWorker},
	 * graticule only on {@code MapPaneListener#onDisplayAreaChanged}, coarser color quantization by N,
	 * and {@code forceMapRepaint} without inline {@code updateGraticule} — caused pan/zoom display issues.</p>
	 */
	public void updateMultipleCatalogsDisplay() {
		if (cachedSymbolSizeFromSettings < 0) {
			cachedSymbolSizeFromSettings = com.treloc.xtreloc.app.gui.util.AppSettingsCache.snapshot().getSymbolSize();
		}
		this.symbolSize = Math.max(1, Math.min(50, cachedSymbolSizeFromSettings));
		...
		try {
			...
			connectionType = DataUtilities.createType("Connection", 
				"geom:LineString");
			errorBarType = DataUtilities.createType("ErrorBar", "geom:LineString");
		} catch (Exception e) {
		...
		for (com.treloc.xtreloc.app.gui.model.CatalogInfo catalogInfo : catalogInfos) {
			if (!catalogInfo.isVisible() || catalogInfo.getHypocenters() == null || !catalogInfo.isShowErrorEllipse()) continue;
			for (Hypocenter h : catalogInfo.getHypocenters()) {
				if (!isFiniteLonLat(h.lon, h.lat)) {
					continue;
				}
				if (h.xerr > 0 || h.yerr > 0) {
					org.locationtech.jts.geom.Polygon errorEllipse = createErrorEllipse(h.lon, h.lat, h.xerr, h.yerr, gf);
					org.locationtech.jts.geom.LineString outline = errorEllipseOutlineOrNull(errorEllipse);
					if (outline != null) {
						SimpleFeature errorFeature = DataUtilities.template(errorBarType);
						errorFeature.setDefaultGeometry(outline);
		...
		if (colorMapCatalog != null && colorMapValues != null && 
			...
			double[] autoRange = ColorScaleUtils.computeAutoColorRange(colorMapValues, 0.02, 0.98);
			minValue = autoRange[0];
			maxValue = autoRange[1];
			...
			for (int i = 0; i < catalogHypocenters.size() && i < colorMapValues.length; i++) {
				...
				Color color;
				if (!Double.isFinite(colorValue)) {
					color = MAP_COLOR_VALUE_MISSING;
				} else {
					float normalized = normalizedColorRatio(colorValue, minValue, maxValue);
					color = getColorFromPalette(normalized);
				}
				...
				for (java.util.Map.Entry<String, java.util.Map<com.treloc.xtreloc.app.gui.model.CatalogInfo, Hypocenter>> entry : timeToCatalogHypocenter.entrySet()) {
					...
					if (catalogHypocenterMap.size() >= 2) {
						...
						for (int i = 0; i < catalogsForTime.size() - 1; i++) {
							...
							if (!isFiniteLonLat(h1.lon, h1.lat) || !isFiniteLonLat(h2.lon, h2.lat)) {
								continue;
							}
							...
							if (!isRenderableLineString(line)) {
								continue;
							}
	...
	private void updateLegend() {
		...
		for (com.treloc.xtreloc.app.gui.model.CatalogInfo catalogInfo : catalogInfos) {
			if (catalogInfo.isVisible() && catalogInfo.getHypocenters() != null && !catalogInfo.getHypocenters().isEmpty()) {
				...
				Color rawLegendColor = catalogInfo.getColor();
				final Color color = rawLegendColor != null ? rawLegendColor : new Color(96, 96, 96);
				com.treloc.xtreloc.app.gui.model.CatalogInfo.SymbolType symbolType = catalogInfo.getSymbolType();
	...
	public void showHypocenters(List<Hypocenter> hypos, String colorColumn, double[] colorValues) throws Exception {
		...
		double minValue;
		double maxValue;
		double[] valsForRange = new double[hypos.size()];
		for (int i = 0; i < hypos.size(); i++) {
			valsForRange[i] = (colorValues != null && i < colorValues.length) 
				? colorValues[i] 
				: hypos.get(i).depth;
		}
		double[] autoRange = ColorScaleUtils.computeAutoColorRange(valsForRange, 0.02, 0.98);
		minValue = autoRange[0];
		maxValue = autoRange[1];
		...
		for (int i = 0; i < hypos.size(); i++) {
			boolean hasColorColumn = colorValues != null && i < colorValues.length;
			double colorValue = hasColorColumn ? colorValues[i] : hypos.get(i).depth;
			Color color;
			if (hasColorColumn && !Double.isFinite(colorValues[i])) {
				color = MAP_COLOR_VALUE_MISSING;
			} else {
				float normalized = normalizedColorRatio(colorValue, minValue, maxValue);
				color = getColorFromPalette(normalized);
		...
		SimpleFeatureType errorBarType = DataUtilities.createType("ErrorBar", "geom:LineString");
		DefaultFeatureCollection errorBarCollection = new DefaultFeatureCollection("ErrorBarLayer", errorBarType);
		...
		for (int i = 0; i < hypos.size(); i++) {
			...
			if (showErrorEllipse && (h.xerr > 0 || h.yerr > 0) && isFiniteLonLat(h.lon, h.lat)) {
				org.locationtech.jts.geom.Polygon errorEllipse = createErrorEllipse(h.lon, h.lat, h.xerr, h.yerr, gf);
				org.locationtech.jts.geom.LineString outline = errorEllipseOutlineOrNull(errorEllipse);
				if (outline != null) {
					SimpleFeature errorFeature = DataUtilities.template(errorBarType);
	...
	/**
	 * Exterior ring of an error ellipse as a line for stroke-only map rendering.
	 */
	private static org.locationtech.jts.geom.LineString errorEllipseOutlineOrNull(org.locationtech.jts.geom.Polygon poly) {
		if (poly == null) {
			return null;
		}
		org.locationtech.jts.geom.LineString ring = poly.getExteriorRing();
		if (ring == null || ring.isEmpty() || ring.getNumPoints() < 4) {
			return null;
		}
		if (!isRenderableLineString(ring)) {
			return null;
		}
		return ring;
	}
	 ...
	 */
	private static final double MAP_LINE_MIN_LEN = 1e-12;

	private static boolean isFiniteLonLat(double lon, double lat) {
		return Double.isFinite(lon) && Double.isFinite(lat)
			&& Math.abs(lon) <= 360.0 && Math.abs(lat) <= 90.0;
	}

	private static boolean isRenderableLineString(LineString line) {
		if (line == null || line.isEmpty() || line.getNumPoints() < 2) {
			return false;
		}
		return line.getLength() > MAP_LINE_MIN_LEN;
	}
	...
	private org.locationtech.jts.geom.Polygon createErrorEllipse(double lon, double lat, double xerr, double yerr, GeometryFactory gf) {
		...
		final double MIN_ERR_KM = 1e-6;
		double xe = Math.max(xerr, MIN_ERR_KM);
		double ye = Math.max(yerr, MIN_ERR_KM);
		double latRad = Math.toRadians(lat);
		double cosLat = Math.cos(latRad);
		if (!Double.isFinite(cosLat) || Math.abs(cosLat) < 1e-5) {
			cosLat = cosLat >= 0 ? 1e-5 : -1e-5;
		}
		double lonErrDeg = xe / (DEG2KM * cosLat);
		double latErrDeg = ye / DEG2KM;
		if (!Double.isFinite(lon) || !Double.isFinite(lat)
			|| !Double.isFinite(lonErrDeg) || !Double.isFinite(latErrDeg)
			|| lonErrDeg <= 0 || latErrDeg <= 0) {
			return null;
		}
		...
		if (!poly.isValid()) {
			try {
				...
				if (fixed instanceof org.locationtech.jts.geom.Polygon) {
					poly = (org.locationtech.jts.geom.Polygon) fixed;
				} else {
					return null;
				}
	...
	private static float normalizedColorRatio(double value, double minValue, double maxValue) {
		if (!Double.isFinite(value)) {
			return 0.5f;
		}
		double span = maxValue - minValue;
		double r = span > 0.0 ? (value - minValue) / span : 0.5;
		if (r < 0.0) {
			r = 0.0;
		} else if (r > 1.0) {
			r = 1.0;
		...
		switch (currentPalette) {
			...
			case GRAYSCALE: {
				...
				if (rr < 0f) {
					...
					rr = 1f;
				} else if (Float.isNaN(rr)) {
					rr = 0f;
				}
				int gray = Math.max(0, Math.min(255, Math.round(255 * rr)));
	...
	private void setupMapOverlays() {
		SwingUtilities.invokeLater(() -> {
			try {
				...
				logger.log(Level.WARNING, "Failed to setup map overlays: " + e.getMessage(), e);
			}
		});
	}
	...
	private void setupStatusBarExportButton() {
		SwingUtilities.invokeLater(() -> {
			try {
				...
				// Error ellipse toggle (icon): OFF = greyed, ON = normal
				ImageIcon errorIconOn = loadIconFromResource("/images/Error.png");
				ImageIcon errorIconOff = errorIconOn != null ? greyedIcon(errorIconOn) : null;
				if (errorIconOn != null && errorIconOff != null) {
					if (errorEllipseToggleButton != null && errorEllipseToggleButton.getParent() != null) {
						errorEllipseToggleButton.getParent().remove(errorEllipseToggleButton);
					}
					errorEllipseToggleButton = new JToggleButton(errorIconOff);
					errorEllipseToggleButton.setSelected(showErrorEllipse);
					errorEllipseToggleButton.setIcon(showErrorEllipse ? errorIconOn : errorIconOff);
					errorEllipseToggleButton.setSelectedIcon(errorIconOn);
					errorEllipseToggleButton.setToolTipText("<html>Error ellipse<br>Show/hide confidence ellipses for hypocenter locations</html>");
					errorEllipseToggleButton.setBorderPainted(false);
					errorEllipseToggleButton.setContentAreaFilled(false);
					errorEllipseToggleButton.setFocusPainted(false);
					errorEllipseToggleButton.addActionListener(e -> {
						showErrorEllipse = errorEllipseToggleButton.isSelected();
						errorEllipseToggleButton.setIcon(showErrorEllipse ? errorIconOn : errorIconOff);
						setShowErrorEllipse(showErrorEllipse);
					});
					if (statusBar != null && errorEllipseToggleButton.getParent() != statusBar) {
						if (errorEllipseToggleButton.getParent() != null) {
							errorEllipseToggleButton.getParent().remove(errorEllipseToggleButton);
						}
						statusBar.add(errorEllipseToggleButton);
					}
				}

				// Event connection toggle (icon): OFF = greyed, ON = normal
				ImageIcon connectionIconOn = loadIconFromResource("/images/Connection.png");
				ImageIcon connectionIconOff = connectionIconOn != null ? greyedIcon(connectionIconOn) : null;
				if (connectionIconOn != null && connectionIconOff != null) {
					if (eventConnectionToggleButton != null && eventConnectionToggleButton.getParent() != null) {
						eventConnectionToggleButton.getParent().remove(eventConnectionToggleButton);
					}
					eventConnectionToggleButton = new JToggleButton(connectionIconOff);
					eventConnectionToggleButton.setSelected(showConnections);
					eventConnectionToggleButton.setIcon(showConnections ? connectionIconOn : connectionIconOff);
					eventConnectionToggleButton.setSelectedIcon(connectionIconOn);
					eventConnectionToggleButton.setToolTipText("<html>Event connection<br>Show/hide lines connecting related events</html>");
					eventConnectionToggleButton.setBorderPainted(false);
					eventConnectionToggleButton.setContentAreaFilled(false);
					eventConnectionToggleButton.setFocusPainted(false);
					eventConnectionToggleButton.addActionListener(e -> {
						showConnections = eventConnectionToggleButton.isSelected();
						eventConnectionToggleButton.setIcon(showConnections ? connectionIconOn : connectionIconOff);
						setShowConnections(showConnections);
					});
					if (statusBar != null && eventConnectionToggleButton.getParent() != statusBar) {
						if (eventConnectionToggleButton.getParent() != null) {
							eventConnectionToggleButton.getParent().remove(eventConnectionToggleButton);
						}
						statusBar.add(eventConnectionToggleButton);
					}
				}

				// Reload map button: re-build map layers from current data
				ImageIcon reloadIcon = loadIconFromResource("/images/Reload.png");
				if (reloadIcon != null) {
					if (reloadMapButton != null && reloadMapButton.getParent() != null) {
						reloadMapButton.getParent().remove(reloadMapButton);
					}
					reloadMapButton = new JButton(reloadIcon);
					reloadMapButton.setToolTipText("<html>Reload map<br>Refresh map display and re-draw layers</html>");
					reloadMapButton.setBorderPainted(false);
					reloadMapButton.setContentAreaFilled(false);
					reloadMapButton.setFocusPainted(false);
					reloadMapButton.addActionListener(e -> reloadMap());
					if (statusBar != null && reloadMapButton.getParent() != statusBar) {
						if (reloadMapButton.getParent() != null) {
							reloadMapButton.getParent().remove(reloadMapButton);
						}
						statusBar.add(reloadMapButton);
					}
				}
				...
				if (saveBundled != null && saveBundled.getImage() != null) {
					...
					exportImageButton.setToolTipText("<html>Save figure<br>Export the current map as an image file (PNG/JPEG)</html>");
					...
					exportImageButton.setToolTipText("<html>Save figure<br>Export the current map as an image file</html>");
					exportImageButton.addActionListener(e -> exportMapImage());
				}
				...
				logger.log(Level.WARNING, "Failed to setup status bar export button: " + e.getMessage(), e);
				if (exportImageButton != null) {
					try {
						...
						logger.log(Level.WARNING, "Failed to add export button to toolbar: " + ex.getMessage(), ex);
					}
				}
			}
		});
	...
	/** Loads an icon from the classpath (e.g. /images/Error.png). Returns null if not found. */
	private ImageIcon loadIconFromResource(String path) {
		java.net.URL url = MapView.class.getResource(path);
		if (url == null) return null;
		ImageIcon icon = new ImageIcon(url);
		Image img = icon.getImage();
		if (img == null || img.getWidth(null) <= 0) return null;
		Image scaled = img.getScaledInstance(20, 20, Image.SCALE_SMOOTH);
		return new ImageIcon(scaled);
	}

	/** Returns a greyed-out (disabled-style) version of the icon for OFF state. */
	private ImageIcon greyedIcon(ImageIcon icon) {
		if (icon == null) return null;
		Image img = icon.getImage();
		if (img == null) return null;
		Image greyed = javax.swing.GrayFilter.createDisabledImage(img);
	...
	public void showStations(List<Station> stations, String colorColumn, double[] colorValues) throws Exception {
		...
		if (colorColumn != null && colorValues != null && colorValues.length == stations.size()) {
			double[] autoRange = ColorScaleUtils.computeAutoColorRange(colorValues, 0.02, 0.98);
			double minValue = autoRange[0];
			double maxValue = autoRange[1];
			
			java.util.Map<Color, DefaultFeatureCollection> colorFeatureCollections = new java.util.HashMap<>();
			
			for (int i = 0; i < stations.size(); i++) {
				Station s = stations.get(i);
				if (!isFiniteLonLat(s.getLon(), s.getLat())) {
					continue;
				}
				double valueToColor = colorValues[i];
				Color color;
				if (!Double.isFinite(valueToColor)) {
					color = MAP_COLOR_VALUE_MISSING;
				} else {
					float normalized = normalizedColorRatio(valueToColor, minValue, maxValue);
					color = getColorFromPalette(normalized);
				}
	...
	public void highlightPoint(double lon, double lat, String type, 
		...
		try {
			...
			logger.log(Level.WARNING, "Failed to highlight point: " + e.getMessage(), e);
		}
	}
	 ...
	 *
	 * @param size the symbol size (1–50)
	 */
	public void setSymbolSize(int size) {
		this.symbolSize = Math.max(1, Math.min(50, size));
		this.cachedSymbolSizeFromSettings = this.symbolSize;
		updateLegend();
		forceMapRepaint();
	}
					...
					if (labelsVisible && graduation.size() > 0) {
						...
						for (int i = 0; i < graduation.size(); i++) {
							...
							if (denom <= 0.0 || !Double.isFinite(denom)) {
								/* min==max (e.g. all zeros): avoid 0/0; single label centered on bar */
								ratio = graduation.size() <= 1 ? 0.5
									: (double) i / (double) (graduation.size() - 1);
							} else {
								ratio = (value - minValue) / denom;
								if (ratio < 0.0) {
									ratio = 0.0;
								} else if (ratio > 1.0) {
									ratio = 1.0;
