...
public class ConfigInputHelper {
    ...
    private static final Logger LOG = Logger.getLogger(ConfigInputHelper.class.getName());
    ...
    public void createParameterForm(String mode, Runnable onOk, Runnable onCancel, Runnable onRun) {
        createParameterForm(mode, onOk, onCancel, onRun, null);
    }

    /**
     * Creates parameter input form, optionally seeding the form state from the given config
     * so that rmsThreshold, locErrThreshold (CLS) and locErr (SYN) etc. appear in the form.
     *
     * @param mode the mode for which to create configuration
     * @param onOk callback when OK is clicked
     * @param onCancel callback when Cancel is clicked
     * @param onRun optional callback when Run is clicked directly (null to omit)
     * @param seedConfig optional config to seed the form state (e.g. imported or previous run config)
     */
    public void createParameterForm(String mode, Runnable onOk, Runnable onCancel, Runnable onRun, AppConfig seedConfig) {
        ...
        switch (mode.toUpperCase()) {
            ...
            case "CLS":
                ...
                paramPanel.addComponent(new Label("RMS threshold (empty for no filter):"));
                TextBox rmsThresholdBox = new TextBox(new TerminalSize(textBoxWidth, 1));
                rmsThresholdBox.setText("");
                paramPanel.addComponent(rmsThresholdBox);
                modeComponents.add(rmsThresholdBox);
                
                paramPanel.addComponent(new Label("Location error threshold km (empty for no filter):"));
                TextBox locErrThresholdBox = new TextBox(new TerminalSize(textBoxWidth, 1));
                locErrThresholdBox.setText("");
                paramPanel.addComponent(locErrThresholdBox);
                modeComponents.add(locErrThresholdBox);
                break;
        ...
        switch (mode.toUpperCase()) {
            ...
            case "TRD":
                ...
                paramPanel.addComponent(new Label("Max triple-diff count per cluster (empty = no limit, smallest residual first):"));
                TextBox maxTripleDiffCountBox = new TextBox(new TerminalSize(textBoxWidth, 1));
                maxTripleDiffCountBox.setText("");
                paramPanel.addComponent(maxTripleDiffCountBox);
                solverComponents.add(maxTripleDiffCountBox);
                break;
        }
        
        paramPanel.addComponent(new EmptySpace(new TerminalSize(0, 1)));
        
        // Seed form state from config so rms/locErr (CLS) and locErr (SYN) etc. appear when opening form with loaded config
        if (seedConfig != null) {
            seedFormStateFromConfig(mode, seedConfig);
        }
            ...
            switch (mode.toUpperCase()) {
                ...
                case "CLS":
                    if (modeComponents.size() >= 8) {
                        ...
                        TextBox rmsThresholdBox = (TextBox) modeComponents.get(6);
                        TextBox locErrThresholdBox = (TextBox) modeComponents.get(7);
                        ioConfig.catalogFile = cat.getText().trim();
                        ...
                        String rmsStr = rmsThresholdBox.getText().trim();
                        if (!rmsStr.isEmpty()) {
                            try { paramsNode.put("rmsThreshold", Double.parseDouble(rmsStr)); } catch (NumberFormatException e) { /* omit */ }
                        }
                        String locErrStr = locErrThresholdBox.getText().trim();
                        if (!locErrStr.isEmpty()) {
                            try { paramsNode.put("locErrThreshold", Double.parseDouble(locErrStr)); } catch (NumberFormatException e) { /* omit */ }
                        }
                    }
                    break;
                case "SYN":
            ...
            switch (mode.toUpperCase()) {
                ...
                case "TRD":
                    if (solverComponents.size() >= 3) {
                        ...
                        if (solverComponents.size() >= 4) {
                            String maxCountStr = ((TextBox) solverComponents.get(3)).getText().trim();
                            if (!maxCountStr.isEmpty()) {
                                try {
                                    ...
                                    LOG.log(Level.FINE, "TRD maxTripleDiffCount ignored (invalid): " + maxCountStr, e);
                                }
                            }
                        }
    ...
    /**
     * Merges the given config's mode params (e.g. rmsThreshold, locErrThreshold for CLS; locErr for SYN)
     * into the form state cache so that when the parameter form is restored, those values appear.
     */
    private void seedFormStateFromConfig(String mode, AppConfig seedConfig) {
        if (seedConfig == null || seedConfig.getModes() == null) return;
        AppConfig.ModeConfig mc = seedConfig.getModes().get(mode.toUpperCase());
        if (mc == null) return;
        Map<String, String> state = modeFormStateCache.get(mode.toUpperCase());
        ...
        modeFormStateCache.put(mode.toUpperCase(), state);
    }
